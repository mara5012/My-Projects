...
public class SQLRepositoryInchiriere extends GenericRepository<Inchiriere> {
    ...
    public void populateDatabase() throws SQLException {
        ...
        // Selectăm mașinile deja existente pentru a le folosi în închirieri
        String sqlSelectMasina = "SELECT id FROM Masini";
        List<Integer> masiniIds = new ArrayList<>();

        try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sqlSelectMasina)) {
            while (rs.next()) {
                masiniIds.add(rs.getInt("id"));
            }
        }

        if (masiniIds.isEmpty()) {
            System.out.println("Nu există mașini în baza de date pentru a le închiria.");
            return;
        }

        String sqlInsertInchiriere = "INSERT INTO Inchirieri (id_masina, marca, model, data_inceput, data_sfarsit) VALUES (?, ?, ?, ?, ?)";

        try (PreparedStatement preparedStatement = conn.prepareStatement(sqlInsertInchiriere)) {
            // Generăm 100 de închirieri
            for (int i = 0; i < 100; i++) {
                // Alegem aleatoriu o mașină din cele disponibile
                int idMasina = masiniIds.get(random.nextInt(masiniIds.size()));

                // Obținem detalii despre mașina aleasă
                String sqlGetMasinaDetails = "SELECT marca, model FROM Masini WHERE id = ?";
                try (PreparedStatement ps = conn.prepareStatement(sqlGetMasinaDetails)) {
                    ps.setInt(1, idMasina);
                    try (ResultSet rsMasina = ps.executeQuery()) {
                        if (rsMasina.next()) {
                            String marca = rsMasina.getString("marca");
                            String model = rsMasina.getString("model");

                            // Generăm datele pentru începutul și sfârșitul închirierii
                            Date dataInceput = generateRandomDate();
                            Date dataSfarsit = generateRandomDateAfter(dataInceput);

                            // Setăm parametrii pentru insert
                            preparedStatement.setInt(1, idMasina);
                            preparedStatement.setString(2, marca);
                            preparedStatement.setString(3, model);
                            preparedStatement.setDate(4, dataInceput);
                            preparedStatement.setDate(5, dataSfarsit);
                ...
                // La fiecare 10 închirieri, executăm batch-ul și facem commit
                if (i % 10 == 0) {
                    preparedStatement.executeBatch();
                    conn.commit();
                    System.out.println("Am adăugat " + (i + 1) + " închirieri.");
                }
            }

            // Executăm restul batch-ului
            preparedStatement.executeBatch();
            conn.commit();
            System.out.println("Inserarea celor 100 de închirieri a fost finalizată.");
        } catch (SQLException e) {
            // Dacă apare o eroare, facem rollback
            try {
                conn.rollback();  // Anulează tranzacția în caz de eroare
                System.out.println("Rollback efectuat.");
    ...
    private Date generateRandomDate() {
        ...
        long randomTime = currentTime + (long) (random.nextInt(365 * 2) * 24L * 60L * 60L * 1000L);  // între acum și 2 ani în viitor
        return new Date(randomTime);
    }

    private Date generateRandomDateAfter(Date startDate) {
        long startTime = startDate.getTime();
        long randomDuration = random.nextInt(30 * 24 * 60 * 1000);  // 0-30 zile
        return new Date(startTime + randomDuration);
    }
