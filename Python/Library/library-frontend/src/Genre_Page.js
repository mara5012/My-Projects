...
function GenrePage() {
  ...
  const [books, setBooks] = useState(staticBooks); // Start with static books or []
  ...
  useEffect(() => {
    async function fetchBooks() {
      ...
      setError(null);
      try {
        const response = await fetch(`http://localhost:5000/books/search-by-genre?genre=${encodeURIComponent(genreName)}`);
...
>
  {category.name}
      ...
      <div style={{ padding: 24 }}>
        <h2>
         <em>{genreName} Books:</em>
        </h2>

        {books.length > 0 ? (
          <main
  style={{
    display: "grid", // or "block" for list view
